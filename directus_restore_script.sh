...
show_help() {
    ...
    echo "                    If not provided, defaults to '/media/backup/directus_bckp/short_term_bckp'."
...
DEFAULT_BACKUP_DIR="${BACKUP_DIR_LOCAL}/short_term_bckp"
...
if [ -f "$BACKUP_DIR/$BACKUP_ARCHIVE" ]; then
    ...
    if [ "$CONFIRMATION" = "$RESTORE_TIMESTAMP" ]; then
        ...
        tar -xzf "$BACKUP_DIR/$BACKUP_ARCHIVE" -C "$POSTGRES_DIR"

        echo "Directory $POSTGRES_DIR restored to version $RESTORE_TIMESTAMP"
    else
